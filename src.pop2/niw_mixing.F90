...
! !PUBLIC DATA MEMBERS:
   ...
   real (r8), dimension(nx_block,ny_block,max_blocks_clinic), &!added by ym
       public, target ::&
        TEMP2D

   real (r8), dimension(nx_block,ny_block,km,max_blocks_clinic), &!added by ym
        public, target ::&
         KVNIW_init,N2_other,POP_N2,TEMP3D 
...
!
!-----------------------------------------------------------------------
   if (my_task == master_task) write(stdout,*) 'i am here 290'
   if (niw_energy_file(1:7) == 'unknown' .and. niw_energy_type .ne. 'MFP') then
    if (my_task == master_task) write(stdout,*) 'i am here 292'
   ...
   if (my_task == master_task) write(stdout,*) 'i am here 297'
...
!-----------------------------------------------------------------------
     ...
     do iblock = 1,nblocks_clinic
       ...
       NIW_COEF(:,:,iblock) = niw_mixing_efficiency / rho_fw
     enddo
...
! !INTERFACE:
 ...
 subroutine niw_mix(DBLOC, HBLT, KBL, En, En_MFP, VISC, VDC, this_block)
...
!       where (WORK2 > c0)
!	       if (niw_energy_type .eq. 'MFP') then
!		       WORK3(:,:,k) = En_MFP(:,:,k) /WORK2
!		   else 
!               WORK3(:,:,k) = En(:,:) /WORK2
!		    endif
!       endwhere
      if (niw_energy_type .eq. 'MFP') then
          ...
          !    WORK3(:,:,k) = En_MFP(:,:,k) /WORK2
          !endwhere
          where (POP_N2(:,:,k,bid) > c0)
              WORK3(:,:,k) = En_MFP(:,:,k) / POP_N2(:,:,k,bid)
          endwhere
      else
          where (WORK2>c0)
             WORK3(:,:,k) = En(:,:) /WORK2
     ...
     do k=1,km
       if (niw_energy_type .eq. 'MFP') then
           where (k < KMT(:,:,bid) )
               KVNIW = c0
               KVNIW = WORK3(:,:,k)
               !KVNIW_init(:,:,k,bid) = WORK3(:,:,k)
           endwhere
          ! KVNIW_init(:,:,k,bid) = KVNIW
       else
           where ( k >= KBL .and. k < KMT(:,:,bid) )
             KVNIW = c0
             where ( WORK1 > c0 )
       ...
       !if(my_task == master_task) write(stdout,*) 'balabala=',exp(-(zw(k) - HBLT)/niw_vert_decay_scale)
       !    if(my_task == master_task) write(stdout,*) 'WORK1=',WORK1
       !    if(my_task == master_task) write(stdout,*) 'all=',exp(-(zw(k) - HBLT)/niw_vert_decay_scale) / WORK1
...
!-----------------------------------------------------------------------
         !KVNIW_init(:,:,k,bid) = KVNIW
         !TEMP3D(:,:,k,bid) = VDC(:,:,k,1)!KVNIW
         KVNIW = max(VDC(:,:,k,1),KVNIW)
         !KVNIW = max(KVNIW_init(:,:,k,bid),TEMP3D(:,:,k,bid))
         !TEMP3D(:,:,k,bid) = KVNIW
         KVNIW_init(:,:,k,bid) = KVNIW
...
!-----------------------------------------------------------------------
     ...
     do k=1,km
       if (niw_energy_type .ne. 'MFP') then
         where ( k < KBL )

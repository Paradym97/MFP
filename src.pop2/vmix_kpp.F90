...
! !PUBLIC DATA MEMBERS:
   ...
   real (r8), dimension(nx_block,ny_block,km,max_blocks_clinic), &
       public, target ::&
        KPP_KVMIX,          &! vertical diffusivity
        KPP_KVMIX_M,        &! vertical viscosity
        UI_VEL,VI_VEL,POP_S2,POP_S2N2,POP_epsilon,WORK_MFP,WORK_GM,U1,U7,&
        diff_ri,diff_dd,diff_mfp,diff_all,vdc_mfp,vdc_ri,vdc_dd,vdc_bl,KPP_VVC          
!   real (r8), dimension(nx_block,ny_block,max_blocks_clinic), &
!       public, target ::&
!        TEMP2D
    
   real (r8), dimension(nx_block,ny_block,km,7,max_blocks_clinic), &!added by ym
       public, target ::&
   old_UVEL,old_VVEL

   real (r8), dimension(km,201), &!added by ym
       public, target ::&
           S2_mat_dz
   real (r8), dimension(201), &!added by ym
       public, target ::&
           mc_dz

!-----------------------------------------------------------------------
!
!  other module variables  !added by yuanm
!
!-----------------------------------------------------------------------

   integer (int_kind),public :: &
            mfp_flag,      &  ! time flag for mfp
            mfp_freq_iopt, &  ! frequency option for mfp
            mfp_freq          ! frequency of mfp
   
   logical (log_kind), public :: &! added by yman
            lmfp     ! true if time to mfp
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) ::   &
      ...
      tavg_vdc_ri,tavg_vvc_ri,tavg_vdc_t_dd,tavg_vdc_s_dd,tavg_diff_ri,tavg_diff_dd,tavg_diff_bl,&
      tavg_mfp,tavg_gm,tavg_n2,tavg_Ui,tavg_Vi,&
      tavg_TPOWER
   ...
   integer (int_kind) ::   &
      ...
      tavg_Ep !epsilon for MFP by yman 
...
call FP_S2_kc_relation_dz(S2_mat_dz,mc_dz)
...
!-----------------------------------------------------------------------
   ...
   call define_tavg_field(tavg_vdc_t_dd,'VDC_T_DD',3,               &
                          long_name='VDC T DD',             &
                          units='centimeter^2/s',             &
                          grid_loc='3113',                    &
                          coordinates  ='TLONG TLAT z_w_bot time' )    

   call define_tavg_field(tavg_vdc_s_dd,'VDC_S_DD',3,               &
                          long_name='VDC S DD',             &
                          units='centimeter^2/s',             &
                          grid_loc='3113',                    &
                          coordinates  ='TLONG TLAT z_w_bot time' )                        

   call define_tavg_field(tavg_vdc_ri,'VDC_Ri',3,               &
                          long_name='Ri VDC',             &
                          units='centimeter^2/s',             &
                          grid_loc='3113',                    &
                          coordinates  ='TLONG TLAT z_w_bot time' ) 

   call define_tavg_field(tavg_diff_ri,'Diff_Ri',3,               &
                          long_name='Ri diff',             &
                          units='centimeter^2/s',             &
                          grid_loc='3113',                    &
                          coordinates  ='TLONG TLAT z_w_bot time' )
                          
   call define_tavg_field(tavg_diff_dd,'Diff_dd',3,               &
                          long_name='dd diff',             &
                          units='centimeter^2/s',             &
                          grid_loc='3113',                    &
                          coordinates  ='TLONG TLAT z_w_bot time' )

   call define_tavg_field(tavg_diff_bl,'Diff_bl',3,               &
                          long_name='bl diff',             &
                          units='centimeter^2/s',             &
                          grid_loc='3113',                    &
                          coordinates  ='TLONG TLAT z_w_bot time' )

   call define_tavg_field(tavg_mfp,'Diff_mfp',3,               &
                          long_name='mfp',             &
                          units='centimeter^2/s',             &
                          grid_loc='3113',                    &
                          coordinates  ='TLONG TLAT z_w_bot time' )
                          
   call define_tavg_field(tavg_gm,'Diff_gm',3,               &
                          long_name='gm',             &
                          units='centimeter^2/s',             &
                          grid_loc='3113',                    &
                          coordinates  ='TLONG TLAT z_w_bot time' )
                          
   call define_tavg_field(tavg_n2,'mfp_n2',3,               &
                          long_name='n2',             &
                          units='centimeter^2/s',             &
                          grid_loc='3113',                    &
                          coordinates  ='TLONG TLAT z_w_bot time' )  
   ...
   call define_tavg_field(tavg_vvc_ri,'VVC_Ri',3,               &
                          long_name='Ri VVC ',             &
                          units='centimeter^2/s',             &
                          grid_loc='3113',                    &
                          coordinates  ='TLONG TLAT z_w_bot time' )
   ...
   if (lniw_mixing) then
     ...
     call define_tavg_field(tavg_En,'En',2,                       &
                            ...
                            units='Watts/meter^2',                &
                            grid_loc='2221')
     call define_tavg_field(tavg_Ep,'Ep',3,                       &
                            long_name='epsilon for MFP ',&!by yman
                            units='Watts/meter^2',                &
                            grid_loc='2221')

     call define_tavg_field(tavg_Ui,'Ui',3,                       &
                            long_name='Ui for MFP ',&!by yman
                            units='cm/s',                &
                            grid_loc='3221')
     call define_tavg_field(tavg_Vi,'Vi',3,                       &
                            long_name='Vi for MFP ',&!by yman
                            units='cm/s',                &
                            grid_loc='3221')
...
!-----------------------------------------------------------------------
!
!  initial time flag of MFP !added by yuanm
!
!-----------------------------------------------------------------------

   mfp_freq_iopt = freq_opt_nhour
   mfp_freq      = 1

   call init_time_flag('mfp',mfp_flag, default=.false.,  &
                       owner    = 'init_mfp',               & 
                       freq_opt = mfp_freq_iopt,&
                       freq     = mfp_freq)
...
!-----------------------------------------------------------------------
   ...
   real (r8) ::  &
    ...
    real (r8), dimension(nx_block,ny_block,km) :: Ui,Vi,En_MFP
...
!-----------------------------------------------------------------------
!
!  check time to mfp !added by yuanm
!
!-----------------------------------------------------------------------

   lmfp = .false.
   if (niw_energy_type == 'MFP') then
      lmfp = check_time_flag(mfp_flag)
      if (lcalmfp) then
         lniw_mixing = .true.
      else
         lniw_mixing = .false.
   ...
   !if (my_task == master_task)  write(stdout,*) "lmfp=",lmfp
   !if (my_task == master_task)  write(stdout,*) "lcalmfp=",lcalmfp
   !if (my_task == master_task)  write(stdout,*) "lniw_mixing=",lniw_mixing
...
!-----------------------------------------------------------------------
     ...
     call compute_niw_energy_flux(VISC,VDC,UUU,VVV,KE_mix,UCUR,VCUR,KE_cur,  &
      ...
      diff_mfp(:,:,:,bid)=VDC(:,:,1:km,1)
   endif 
   vdc_mfp(:,:,:,bid)=VDC(:,:,1:km,1)
   ! if (lniw_mixing) then
   !if (my_task == master_task) then
   !   write(stdout,*) 'js in vmix=',js
   !endif
   !if (js>7) then 
   !    call compute_ui_vi(Ui,Vi,this_block)
   !    call compute_MFP_En(En_MFP,Ui,Vi,DBLOC,this_block)
   !endif
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
   ...
   do k=1,km
      call accumulate_tavg_field(VDC(:,:,k,1),tavg_diff_ri,bid,k)
   enddo
   vdc_ri(:,:,:,bid)=VDC(:,:,1:km,1)
...
!-----------------------------------------------------------------------
   ...
   do k=1,km
      call accumulate_tavg_field(VDC(:,:,k,1),tavg_diff_dd,bid,k)
   enddo
   
   vdc_dd(:,:,:,bid)=VDC(:,:,1:km,1)
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
   ...
   do k=1,km
      call accumulate_tavg_field(VDC(:,:,k,1),tavg_diff_bl,bid,k)
   enddo
   vdc_bl(:,:,:,bid)=VDC(:,:,1:km,1)
!-----------------------------------------------------------------------
!
...
!   if (my_task == master_task) then
!       write(stdout,*) 'convect_diff=',convect_diff
!       write(stdout,*) 'convect_visc=',convect_visc
!   endif 
...
!                            + (zt(k-1) - p5*DZT(:,:,k-1,bid))*BFSFC
   ...
   diff_all(:,:,:,bid)=VDC(:,:,1:km,1)
   KPP_VVC(:,:,:,bid)=VVC(:,:,1:km)
!-----------------------------------------------------------------------
!EOC
...
!-----------------------------------------------------------------------
      ...
      U7(:,:,k,bid) = DBLOC(:,:,k)/(p5*(DZT(:,:,k,bid) + DZT(:,:,k+1,bid)))
...
! use DS product point to test, global (i,j) = (5,103)
  ...
  200 format(' Rich  i,j,k DIFF =',3(i3,1x),1pe11.4,1x)
        ...
        if (lrich) then
           FRI    = min((max(WORK0(:,:,k),c0))/Riinfty, c1)
           WORK_ri = rich_mix*(c1 - FRI*FRI)**3 !by yman
           VISC(:,:,k  ) = VISC(:,:,k) + &
           ...
           if ( k < km ) then
              ...
              call accumulate_tavg_field(WORK_ri,tavg_vdc_ri,bid,k)
              diff_ri(:,:,k,bid)=WORK_ri
           endif
           call accumulate_tavg_field(WORK_ri,tavg_vvc_ri,bid,k)
...
!
!-----------------------------------------------------------------------
      !call accumulate_tavg_field(VDC(:,:,k,1),tavg_diff_ri,bid,k) !by yman
   end do
...
! x4 because for 16 hours 1/f a time step of 1 hours gets only a quarter of max. increase
   ...
   do kl = 2,km
      ...
      if ( lniw_mixing .and. niw_energy_type .ne. 'MFP') then !by yman
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) ::  k,kup,knxt,bid
...
!-----------------------------------------------------------------------
      ...
      WORK1 = WORK1 + DIFFDD
      WORK2 = WORK2 + PRANDTL*DIFFDD

      call accumulate_tavg_field(WORK1,tavg_vdc_t_dd,bid,k)
      call accumulate_tavg_field(WORK2,tavg_vdc_s_dd,bid,k)
      diff_dd(:,:,k,bid)=WORK1
   end do
...
!-----------------------------------------------------------------------
   ...
   !real (r8), dimension(km,201) ::  S2_mat,mc  
...
!-----------------------------------------------------------------------
       if(my_task == master_task) write(stdout,*) 'i am fine in 3480'
       call blke(UUU,VVV,KBL,KE_mix)

!-----------------------------------------------------------------------
!  compute boundary layer kinetic energy for cur (current) time
!-----------------------------------------------------------------------
       if(my_task == master_task) write(stdout,*) 'i am fine in 3486'
       call blke(UCUR,VCUR,KBL,KE_cur)
       if(my_task == master_task) write(stdout,*) 'i am fine in 3488'
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
   ...
   else if (niw_energy_type .eq. 'MFP') then
        if (lmfp .and. lcalmfp) then
          if (my_task == master_task) write(stdout,*) 'i did mfp'
          call compute_ui_vi_new(Ui,Vi,this_block)
          call compute_MFP_En(En_MFP,Ui,Vi,DBLOC,KBL,this_block)
        else
          En_MFP=POP_epsilon(:,:,:,bid)
        endif
        do k= 1, km
            call accumulate_tavg_field(En_MFP(:,:,k)/1e4_r8,tavg_Ep,bid,k)
            call accumulate_tavg_field(WORK_MFP(:,:,k,bid),tavg_mfp,bid,k)
            call accumulate_tavg_field(WORK_GM(:,:,k,bid),tavg_gm,bid,k)
            call accumulate_tavg_field(POP_N2(:,:,k,bid),tavg_n2,bid,k)            
            call accumulate_tavg_field(UI_VEL(:,:,k,bid),tavg_Ui,bid,k)
            call accumulate_tavg_field(VI_VEL(:,:,k,bid),tavg_Vi,bid,k)
            En_MFP (:,:,k) = En_MFP(:,:,k) * NIW_COEF(:,:,bid)
        enddo
        !call accumulate_tavg_field(En/c1000,tavg_En,bid,1)
...
!-----------------------------------------------------------------------
        ...
        !call compute_ui_vi(Ui,Vi,this_block)
        !call compute_MFP_En(En_MFP,Ui,Vi,DBLOC,this_block)
     call niw_mix(DBLOC, KPP_HBLT, KBL, En, En_MFP, VISC, VDC, this_block)
...
subroutine compute_ui_vi(Ui,Vi,this_block)

! !DESCRIPTION:
!  compute ui&vi with two-step filter
!  zi(tk)=-sz(tk)+sz(tk-1)
!  s=[1-exp(i*f*dt)]^-1
!
! !REVISION HISTORY：
...
!      UUU,VVV            ! velocities at mix time
   type (block), intent(in) :: &
      this_block        ! block information for current block	 
   real (r8), dimension(nx_block,ny_block,km), intent(out) ::  &	
      Ui,Vi                    !near-inertial velocities	  
...
!-----------------------------------------------------------------------
   real (r8), dimension(nx_block,ny_block,km) ::  &
      ...
      DU,DV                    !U(k)-U(k-1)	
   real (r8), dimension(nx_block,ny_block)    ::  &
      theta_f,            &!theta_f=F*dtt
      F,                  &!Coriolis frequency degree/s
      WORK                 ! WORK array
   integer (int_kind) ::    &
      k,                    &! vertical level index
...
!-----------------------------------------------------------------------
   ...
   Vi=c0
  ! if (my_task == master_task) then
  !    write(stdout,*),'in compute_ui_vi'
  !    write(stdout,*),'mixtime=',mixtime
  !    write(stdout,*),'order=',order
  ! endif
   !UUU=UVEL(:,:,:,mixtime,bid);
   !VVV=VVEL(:,:,:,mixtime,bid);
   !DU=UVEL(:,:,:,mixtime,bid)-UVEL(:,:,:,mixtime-1,bid);
   !DV=VVEL(:,:,:,mixtime,bid)-VVEL(:,:,:,mixtime-1,bid);
   !DU=UVEL(:,:,:,curtime,bid)-UVEL(:,:,:,mixtime,bid);
   !DV=VVEL(:,:,:,curtime,bid)-VVEL(:,:,:,mixtime,bid);
   
   !DU=old_UVEL(:,:,:,order(7),bid)-old_UVEL(:,:,:,order(1),bid)
   !DV=old_VVEL(:,:,:,order(7),bid)-old_VVEL(:,:,:,order(1),bid)
   do k = 1, km
      call ugrid_to_tgrid(DU(:,:,k),old_UVEL(:,:,k,order(7),bid)-old_UVEL(:,:,k,order(1),bid),bid)
      call ugrid_to_tgrid(DV(:,:,k),old_VVEL(:,:,k,order(7),bid)-old_VVEL(:,:,k,order(1),bid),bid)
   enddo
!   write(stdout,*) 'size of ULAT'
!   write(stdout,*),shape(UAT(:,:,bid));
!   write(stdout,*),'size of F'
!   write(stdout,*),
   F(:,:) = c2*omega*sin(TLAT(:,:,bid))
   theta_f=F*60.0*(c1+c5)
   WORK=sin(theta_f)/(1-cos(theta_f))
   do k=1,km
      Ui(:,:,k)=p5*(DU(:,:,k)-WORK*DV(:,:,k))
      Vi(:,:,k)=p5*(WORK*DU(:,:,k)+DV(:,:,k))
   enddo
   UI_VEL(:,:,:,bid) = Ui
   VI_VEL(:,:,:,bid) = Vi
!-----------------------------------------------------------------------
...
end subroutine compute_ui_vi

subroutine compute_ui_vi_new(Ui,Vi,this_block)
...
!  v0 7/21/2019
!
! !last modified v2 in 10/10/2019 
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) ::    &
      ...
      j,                    &! latitudinal index
      bid                    ! local block index  
   complex (r8), dimension(nx_block,ny_block,km) :: &
      z,                      &
	  zf,                     &
	  s
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
   Ui=c0
   Vi=c0
   F = c2*omega*sin(ULAT(:,:,bid))
   theta_f=F*3600.*(c1+c5)
   WORK=sin(theta_f)/(1-cos(theta_f))
   do k = 1,km
       s(:,:,k) = 1/(1-exp(cmplx(0,theta_f)))
   enddo
   !TEMP2D(:,:,bid) = sin(ULAT(:,:,bid))
   !if (my_task == master_task) then
   !    write(stdout,*) 'omega=',omega
   !    write(stdout,*) 'sin(ULAT(3,3,bid))'
   !    write(stdout,*) 'dtt=',dtt
   !    write(stdout,*) 'F=',F(3,3)
   !    write(stdout,*) 'ULAT=',ULAT(3,3,bid)
   !    write(stdout,*) 'theta_f=',theta_f(3,3)
   !    write(stdout,*) 'WORK=',WORK(3,3)
   !    write(stdout,*) 's=',aimag(s(3,3,1))
   !endif
   zf = -s*cmplx(old_UVEL(:,:,:,order(1),bid),old_VVEL(:,:,:,order(1),bid))&
        +s*cmplx(old_UVEL(:,:,:,order(7),bid),old_VVEL(:,:,:,order(7),bid))
   Ui = real(zf)
   Vi = aimag(zf)
   !U1(:,:,:,bid)=old_UVEL(:,:,:,order(1),bid)
   !U7(:,:,:,bid)=old_UVEL(:,:,:,order(7),bid)
   UI_VEL(:,:,:,bid) = Ui
   VI_VEL(:,:,:,bid) = Vi
!-----------------------------------------------------------------------
!EOC  

end subroutine compute_ui_vi_new

subroutine FP_S2_kc_relation_dz(S2_mat_dz,mc_dz)
   ! !DESCRIPTION:
   !  This function relates the normalized near-inertial 
   !  shear variance S2/N2 to the critical vertical wavenumber mc. 
   !  Please refer to the Eqs. 3 and 4 in the SR paper.
   !  
   ! !REVISION HISTORY：
   !  v1 8/6/2019
   ! !last modified in 9/9/2019
   
   ! !INPUT PARAMETERS:
      real (r8), dimension(km,201), intent(out) :: &
         S2_mat_dz                 !      
      real (r8), dimension(201), intent(out) :: &
         mc_dz
   !EOP
   !BOC
   !-----------------------------------------------------------------------
   ...
   !-----------------------------------------------------------------------   
      ...
      real (r8), dimension(201) ::&
        theta,                      &!
        a                 
      real (r8), dimension(km,201):: &
      Dm,Dm_m,xm,mc_k,WORK3
   ...
   !
   !-----------------------------------------------------------------------  
      linspace = pi2/(1000.0_r8)
   !   m = (/((i*linspace),i=0,200)/)
      do i = 1,q
         theta(i) = ((i-1))*linspace
      enddo
   
      do k=1,km
          xm(k,:)=theta*dz(k)/200.0_r8!theta*dz/1000/2
          mc_k(k,:)=theta
      enddo
   ...
   !-----------------------------------------------------------------------
       do i=2,q
           !compute trapz(m(k,1:i),Dm(k,1:i))
           WORK1 = sum((mc_k(:,2:i)-mc_k(:,1:i-1))*(Dm(:,2:i)+Dm(:,1:i-1))*p5,2)
           !compute trapz(m(k,i:q),Dm(k,i:q)*mc(i)/m(k,i:q))
           if (i .eq. q) then
               WORK2=c0
           else
               do k=1,km
                   WORK3(k,:)=Dm(k,:)*mc_dz(i)/theta
               enddo
               WORK2 = sum((mc_k(:,i+1:q)-mc_k(:,i:q-1))*(WORK3(:,i+1:q)+WORK3(:,i:q-1))*p5,2)
           endif
           !S_mat(k,i)=a(k,i)*trapz(m(k,1:i),Dm(k,1:i))+&
           !           a(k,i)*trapz(m(k,i:q),Dm(k,i:q)*mc(i)/m(k,i:q))		
           S2_mat_dz(:,i) = a(i)*(WORK1+WORK2)
           
       enddo
       if (my_task == master_task) then
           write(stdout,*) 'S2_mat_dz=',S2_mat_dz(1,:)
           write(stdout,*) 'theta=',theta
           write(stdout,*) 'xm=',xm(1,:)
           write(stdout,*) 'Dm=',Dm(1,:)
           !write(stdout,*) 'Dm_m',Dm_m   
       endif
   end subroutine FP_S2_kc_relation_dz

subroutine FP_S2_kc_relation(S2_mat,mc,dz)
! !DESCRIPTION:
!  This function relates the normalized near-inertial 
!  shear variance S2/N2 to the critical vertical wavenumber mc. 
!  Please refer to the Eqs. 3 and 4 in the SR paper.
...
!   m = (/((i*linspace),i=0,200)/)
   ...
   !do k=1,km
   !    xm(k,:)=theta*dzw(k)/200.0_r8!theta*dz/1000/2
   !    mc_k(k,:)=theta
   !enddo

   xm=theta*dz/200.0_r8
   mc_k=theta
...
!-----------------------------------------------------------------------
    do i=2,q
        !compute trapz(m(k,1:i),Dm(k,1:i))
        WORK1 = sum((mc_k(2:i)-mc_k(1:i-1))*(Dm(2:i)+Dm(1:i-1))*p5)
        !compute trapz(m(k,i:q),Dm(k,i:q)*mc(i)/m(k,i:q))
        if (i .eq. q) then
            WORK2=c0
        else
            !do k=1,km
            !    WORK3(k,:)=Dm(k,:)*mc(i)/theta
            !enddo
            WORK3 = Dm*mc(i)/theta
            WORK2 = sum((mc_k(i+1:q)-mc_k(i:q-1))*(WORK3(i+1:q)+WORK3(i:q-1))*p5)
        endif
        !S_mat(k,i)=a(k,i)*trapz(m(k,1:i),Dm(k,1:i))+&
        !           a(k,i)*trapz(m(k,i:q),Dm(k,i:q)*mc(i)/m(k,i:q))		
        S2_mat(i) = a(i)*(WORK1+WORK2)
        
    enddo
    !if (my_task == master_task) then
    !    write(stdout,*) 'S2_mat=',S2_mat
    !    write(stdout,*) 'mc=',mc
    !    write(stdout,*) 'xm=',xm
    !    write(stdout,*) 'Dm=',Dm
    !    write(stdout,*) 'Dm_m',Dm_m   
    !    write(stdout,*) 'dz=',dz
    !endif
end subroutine FP_S2_kc_relation

subroutine compute_MFP_En(En_MFP,Ui,Vi,DBLOC,KBL,this_block)
! !DESCRIPTION:
!  compute Energy flux with MFP
!  
!  
!
! !REVISION HISTORY：
...
! !INPUT PARAMETERS:
   real (r8), dimension(nx_block,ny_block,km), intent(in) ::  &	
      Ui,Vi,DBLOC                     !near-inertial velocities
	real (r8), dimension(nx_block,ny_block,km),intent(inout) ::  &	 
	  En_MFP                    !
   integer (int_kind), dimension(nx_block,ny_block), intent(in) ::  &
...
!-----------------------------------------------------------------------   
   integer (int_kind) ::    &
      ...
      bid                    ! local block index  
   real (r8), dimension(nx_block,ny_block,km) :: &
   	  B_FRQNCY,N2,             &! buoyancy frequency
      S2,S2_temp,test_MFP                     ! S2=((dUi)/(dz))**2+((dVi)/dz)**2
   real (r8), dimension(nx_block,ny_block,0:km+1) :: S2N2
   real (r8), dimension(201) :: &
	  WORK2,S2_mat,mc
	  
   real (r8), dimension(nx_block,ny_block) ::&
      F,FRI,RI_LOC,VSHEAR!,WORK_MFP,WORK_GM                    ! Coriolis frequency degree/s
   real (r8), dimension(km) :: find_mc,shear_norm,J_norm
   real (r8)  :: &
      N0       ,   &! reference buoyancy frequency
	  mc_GM    ,                  &! critical wavenumber for the GM spectrum
	  f0       ,       &! reference Corilois parameter
	  epsilon0,                  &! reference turbulent dissipation rate
	  Rw,N2_lower,mc_ratio_low,Kp_upper,&
          Gamm,Kp_lower,WORK1,num_v_smooth_S2N2,&
          lat_high,lat_low,wgt1,&
          wgt,nw,lat0
   integer (int_kind) :: mc_loc
   real (r8) :: h1,f_en
...
!----------------------------------------------------------------------- 
      ...
      N0       = pi2*c3/seconds_in_hour
          mc_GM    = pi2/c10                  
          f0       = c2*omega*sin(32.5/radian)       
          epsilon0 = 6.7*eps*c10000                  
          Rw       = c3
          N2_lower = 5e-8_r8
          mc_ratio_low = c1
          Kp_upper = 100_r8
...
!-----------------------------------------------------------------------   
  ...
  S2(:,:,km) = 0
    do k = 1,km
      if(k < km) then
         FRI = (Ui(:,:,k)-Ui(:,:,k+1))**2+(Vi(:,:,k)-Vi(:,:,k+1))**2
         if (partial_bottom_cells) then
            FRI = FRI/(p5*(DZU(:,:,k,bid) + DZU(:,:,k+1,bid)))**2
         endif
      ...
      if (partial_bottom_cells) then
         if(k < km) then
            !RI_LOC = VSHEAR/(DBLOC(:,:,k)+eps)*(p5*(DZT(:,:,k,bid) + DZT(:,:,k+1,bid)))
            S2 (:,:,k) = VSHEAR
            N2 (:,:,k) =  (DBLOC(:,:,k)+eps)/(p5*(DZT(:,:,k,bid) + DZT(:,:,k+1,bid)))
            where(N2(:,:,k) < N2_lower ) N2(:,:,k) = N2_lower 
            RI_LOC = S2(:,:,k)/N2(:,:,k)

         else
            !RI_LOC = VSHEAR/(DBLOC(:,:,k)+eps)*(p5*DZT(:,:,k,bid))
            S2 (:,:,k) = VSHEAR
            N2 (:,:,k) = (DBLOC(:,:,k)+eps)/(p5*DZT(:,:,k,bid))
            where(N2(:,:,k) < N2_lower ) N2(:,:,k) = N2_lower 
            RI_LOC = S2(:,:,k)/N2(:,:,k)
         endif
      else
         RI_LOC = VSHEAR / (DBLOC(:,:,k)+eps) / (zgrid(k)-zgrid(k+1))
         S2 (:,:,k) = VSHEAR / (zgrid(k)-zgrid(k+1))**2
         N2 (:,:,k) = (DBLOC(:,:,k)+eps) / (zgrid(k)-zgrid(k+1))
      endif
      !where(N2(:,:,k) < N2_lower .and. k < KMT(:,:,bid)) N2(:,:,k) = N2_lower 
         S2N2(:,:,k) = merge(RI_LOC, S2N2(:,:,k-1), k < KMT(:,:,bid))
    enddo! k
    !where(N2 < N2_lower ) N2 = N2_lower 
    !call ugrid_to_tgrid(S2,S2_temp,bid)
    POP_N2 (:,:,:,bid) = N2
    POP_S2(:,:,1:km,bid) = S2 
    U1(:,:,:,bid) = S2N2(:,:,1:km)
   ...
   do n = 1,num_v_smooth_S2N2
      ...
      do k=1,km
         ...
         !DIR$ NODEP
         do i=1,nx_block
            RI_LOC(i,j) = S2N2(i,j,k)
            if (KMT(i,j,bid) >= 3) then
               S2N2(i,j,k) = FRI(i,j) + p5*RI_LOC(i,j) + p25*S2N2(i,j,k+1)
            endif
            FRI(i,j) = p25*RI_LOC(i,j)
         enddo!i
         enddo!j
...
!-----------------------------------------------------------------------
!
!  compute mc with FP_S2_kc_relation
!
!-----------------------------------------------------------------------  	  
	  
    !call FP_S2_kc_relation(S2_mat,mc)
   ...
   do k = 1,km
    ...
    do j = 1, ny_block
        ...
        do i = 1, nx_block
            ...
            WORK1=S2N2(i,j,k)
            !POP_S2N2(i,j,k,bid)=WORK1
            !if (partial_bottom_cells) then
            !   !call FP_S2_kc_relation(mc,S2_mat,min(DZU(i  ,j  ,k,bid),&
            !   !                                  DZU(i+1,j  ,k,bid),&
            !   !                                  DZU(i  ,j+1,k,bid),&
            !   !                                  DZU(i+1,j+1,k,bid)))
            !   call FP_S2_kc_relation(mc,S2_mat,DZT(i,j,k,bid))
            !else
            !   call FP_S2_kc_relation(mc,S2_mat,zgrid(k)-zgrid(k+1))
            !endif

            !if ((WORK1(k) >0.3) ) then
               !do qi =1,q
               !   WORK2(qi) = abs(S2_mat(qi) - WORK1)
               !enddo
               !mc_loc = minloc(WORK2,1)

            S2_mat = S2_mat_dz(k,:)
            mc = mc_dz
               mc_loc = minloc(abs(S2_mat-WORK1),1)
               find_mc(k) = mc(mc_loc)
               shear_norm(k) = mc_GM/find_mc(k)
               if(shear_norm(k)<mc_ratio_low) then
                     shear_norm(k) = mc_ratio_low
               endif
               h1=c3/(c2*sqrt(c2))*(Rw+1)/(Rw*sqrt(Rw-1))
               f_en=c2*omega*sin(abs(TLAT(i,j,bid)))
               J_norm(k)=(f_en/f0)*(acosh(sqrt(N2(i,j,k))/f_en)/acosh(N0/f0))
               En_MFP(i,j,k)=epsilon0*N2(i,j,k)/(N0**2)*h1*J_norm(k)*shear_norm(k)**2

               if(En_MFP(i,j,k)*Gamm/N2(i,j,k) >Kp_upper) then
                     En_MFP(i,j,k)=kp_upper*N2(i,j,k)/Gamm
               endif

               WORK_MFP(i,j,k,bid)=En_MFP(i,j,k)
               WORK_GM(i,j,k,bid)=epsilon0*N2(i,j,k)/(N0**2)*h1*J_norm(k)*mc_ratio_low**2
               
               if(WORK_GM(i,j,k,bid)*Gamm/N2(i,j,k)<Kp_lower) then
                       WORK_GM(i,j,k,bid)=kp_lower*N2(i,j,k)/Gamm
               endif


               wgt=1/sqrt(1+(TLATD(i,j,bid)/lat0)**(-2*nw))
               En_MFP(i,j,k)=En_MFP(i,j,k)*wgt+WORK_GM(i,j,k,bid)*(1-wgt)

               if(En_MFP(i,j,k)*Gamm/N2(i,j,k)<Kp_lower) then
                       En_MFP(i,j,k)=kp_lower*N2(i,j,k)/Gamm
               endif
         ...
         enddo !i
    enddo!j
    !call accumulate_tavg_field(WORK_MFP*20.0,tavg_mfp,bid,k)
    !call accumulate_tavg_field(WORK_GM*20.0,tavg_gm,bid,k)
    !call accumulate_tavg_field(B_FRQNCY(:,:,k)*20.0,tavg_n2,bid,k)
   enddo!k
        test_MFP = 0.0
        !POP_epsilon(:,:,:,bid) = max(En_MFP,test_MFP)
        POP_epsilon(:,:,:,bid) = En_MFP
        POP_S2N2(:,:,:,bid) = S2N2(:,:,1:km)

end subroutine compute_MFP_En

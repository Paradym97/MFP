...
! !USES:
   ...
   use overflows
   use overflow_type
   use vmix_kpp !added by ym
   use niw_mixing !added by yuanm
...
!   logical (log_kind) :: &! added by yuanm
...
!-----------------------------------------------------------------------
!
!  put data into old_UVEL
!
!-----------------------------------------------------------------------

   !if(js < 8) then
   !    old_UVEL(:,:,:,js,:) = UVEL(:,:,:,curtime,:)
   !    old_VVEL(:,:,:,js,:) = VVEL(:,:,:,curtime,:)
   !else
   !    temp1 = order(1)
   !    order(1:6) = order(2:7)
   !    order(7) = temp1
   !    old_UVEL(:,:,:,temp1,:) = UVEL(:,:,:,curtime,:)
   !    old_VVEL(:,:,:,temp1,:) = VVEL(:,:,:,curtime,:)
   !endif
   ...
   if (niw_energy_type == 'MFP') then
      if (lmfp) then
         if (.not. lcalmfp) then
            !if (order(7) == 7) lcalmfp =.true.
            do lc = 1, 7
               if (order(lc) == 0) then
                  order(lc) = lc
                  old_UVEL(:,:,:,lc,:) = UVEL(:,:,:,curtime,:)
                  old_VVEL(:,:,:,lc,:) = VVEL(:,:,:,curtime,:)
                  if (lc==7) lcalmfp =.true.
                  exit
               endif
            enddo
         else
...
!   if (my_task == master_task) write(stdout,*) "lniwmixing=",lniw_mixing
...
!-----------------------------------------------------------------------
   ...
   !if(js == 1) then
   !    old_UVEL(:,:,:,1,iblock) = 0
   !    old_UVEL(:,:,:,2,iblock) = UVEL(:,:,:,curtime,iblock)
   !else 
   !    old_UVEL(:,:,:,1,iblock) = old_UVEL(:,:,:,2,iblock)
   !    old_UVEL(:,:,:,2,iblock) = UVEL(:,:,:,curtime,iblock)
   !endif
